...
dim(phi.df02)
head(phi.df02)
pca1 <- prcomp(phi.df02[-1,2:24875], scale=TRUE)
dim(pca1$x)
print(pca1$x[1:10,1:10])
...
# Read xlsx file
raw.data <- read.xlsx("data/GlobalFloodsRecord.xls", sheetName = "MasterTable")
dataFload <- raw.data
# Removed the last 6 columns, they have a blank space.
dataFload<- dataFload[,1:(ncol(dataFload)-6)]
head(dataFload$Main.cause)
# Split tools column into multiple columns
df1 <- cSplit(dataFload, "Main.cause", sep=",")[,lapply(.SD,as.character)]
head(df1)
...
map(database = "world", regions = ".", col="lightgreen", bg="lightblue", fill=TRUE)  #add the adjacent parts of the 
#filter_values=as.double(df1$Centroid.X) & df1$Centroid.Y > 0 & df1$severity >0
#df1.plot=df1[filter_values,]
points(as.double(df1$Centroid.X), as.double(df1$Centroid.Y), pch=19, col="red", cex=(as.double(df1$Severity)/10))  #plot my sample sites
```
...
library(cowplot)
library(gridExtra)
map.data=as.data.frame(cbind(as.double(df1$Centroid.X),as.double(df1$Centroid.Y),as.double(df1$Severity)))
dim(map.data)
colnames(map.data)=c('lon','lat','severity')
continets=c('America','Asia','Europe','Africa')
n <- length(continets)
nCol <- floor(sqrt(n))
plist=vector("list", n)
i=1
for (con in continets){
  map <- get_map(location = con, zoom = 3,source ="google")
  mapPoints <- ggmap(map) +
    geom_point(aes(x = lon, y = lat, size = severity), data = map.data, alpha = .1)
  plist[[i]] <-mapPoints
  i=i+1
}
