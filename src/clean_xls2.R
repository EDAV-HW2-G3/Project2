...
# ==================== Added Part Start ============================= #
dataFlood$BeganYear <- format(dataFlood$Began, "%Y")
dataFlood$BeganMonth <- format(dataFlood$Began, "%m")
dataFlood$EndMonth <- format(dataFlood$End, "%m")
# ==================== Added Part End ============================= #
dataFlood$Began <- format(dataFlood$Began, "%d-%m-%y")
dataFlood$Ended <- format(dataFlood$Ended, "%d-%m-%y")
...
# ==================== Added Part Start ============================= #
dataFlood$Country <- gsub("The Gambia", "Gambia", dataFlood$Country)
dataFlood$Country <- gsub("Comoros islands", "Comoros", dataFlood$Country)
dataFlood$Country <- gsub("Comoros Islands", "Comoros", dataFlood$Country)
dataFlood$Country <- gsub("Alaska", "USA", dataFlood$Country)
dataFlood$Country <- gsub("United Arab Emerates", "United Arab Emirates", dataFlood$Country)
dataFlood$Country <- gsub("England", "UK", dataFlood$Country)
dataFlood$Country <- gsub("Britain", "UK", dataFlood$Country)
dataFlood$Country <- gsub("Scotland", "UK", dataFlood$Country)
dataFlood$Country <- gsub("Britain-Ireland", "UK", dataFlood$Country)
# ==================== Added Part End ============================= #
...
dataFlood$`Main cause` <- gsub("Brief Torrential Rain", "Torrential Rain", dataFlood$`Main cause`)
...
# ==================== Added Part Start ============================= #
as.data.frame(table(df1$`Main cause_1`))
as.data.frame(table(df1$`Main cause_2`))
as.data.frame(table(df1$`Main cause_3`))

#Dam/Levee > Strom, Cyclone, etc ... > Heavy Rain
to_swap <- grepl("Dam/Levee", df1$`Main cause_3`, perl=TRUE)
df1$`Main cause_t`[to_swap] <- df1$`Main cause_1`[to_swap]
df1$`Main cause_1`[to_swap] <- "Dam/Levee"
df1$`Main cause_3`[to_swap] <- df1$`Main cause_t`[to_swap]

to_swap1 <- grepl("Heavy Rain", df1$`Main cause_1`, perl=TRUE)
to_swap2 <- !is.na(df1$`Main cause_2`)
to_swap <- to_swap1 & to_swap2
df1$`Main cause_1`[to_swap] <- df1$`Main cause_2`[to_swap]
df1$`Main cause_2`[to_swap] <- "Heavy Rain"

to_swap1 <- grepl("Snowmelt", df1$`Main cause_2`, perl=TRUE)
to_swap2 <- grepl("Rain", df1$`Main cause_1`, perl=TRUE)
to_swap <- to_swap1 & to_swap2
df1$`Main cause_1`[to_swap] <- "Snowmelt"
df1$`Main cause_2`[to_swap] <- "Rain"

fre <- as.data.frame(table(df1$`Main cause_1`))
to_etc <- as.character(fre$Var1[fre$Freq < 6])
k = 0
for (sm in to_etc){
  to_replace <- grepl(sm, df1$`Main cause_1`, perl=TRUE)
  df1$`Main cause_1`[to_replace] <- "ETC" 
  k = k + length(to_replace[to_replace==TRUE])
}

df1$`Main cause_t` <- NULL # delete the temporary column
...
df1 <- df1[, c(1:25, 29:31, 26:28)]

# ==================== Added Part End ============================= #

# Produce the cleaned data file
write.csv(df1, file= "data/clean_data2.csv", row.names=FALSE)
